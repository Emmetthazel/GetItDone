...
const TodoItems = ({text, id, completed, deleteTask, completeTask, openEditModal, dueDate, getRemainingDays}) => {
  return (
    <div className='border-b border-gray-300 p-1.5 flex items-center'>
        <div onClick={() => completeTask(id)} className='flex flex-1 items-center cursor-pointer'>
            ...
            <span className={`${completed ? 'text-gray-400' : 'text-gray-600'} pl-2`} style={{ textDecoration: completed ? 'line-through' : 'none' }}>{text}{dueDate && <span className="ml-2 text-sm italic text-gray-500">({getRemainingDays(dueDate)})</span>}</span>
        </div>
